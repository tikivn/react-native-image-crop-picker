...
class PickerModule extends ReactContextBaseJavaModule implements ActivityEventListener {
  ...
  private static final String E_DOES_NOT_COMPRESS_VIDEO = "E_DOES_NOT_COMPRESS_VIDEO";
  ...
  private void initiateCamera(Activity activity) {
    ...
    try {
      ...
      if (videoLimitDuration > 0 && mediaType.equals("video")) {
        cameraIntent.putExtra(android.provider.MediaStore.EXTRA_DURATION_LIMIT, videoLimitDuration);
      }
  ...
  public void compressVideo(final ReadableMap options, final Promise promise) {
    ...
    try {
      ...
      new Thread(new Runnable() {
        @Override
        public void run() {
          compression.compressVideo(context, options, path, compressedVideoPath, new PromiseImpl(new Callback() {
            @Override
            ...
            public void invoke(Object... args) {
              ...
              promise.reject(E_DOES_NOT_COMPRESS_VIDEO, ex.getString("message"));
            }
          }));
        }
      }).run();
  ...
  private void getVideo(final Activity activity, final String path, final String mime) throws Exception {
    ...
    new Thread(new Runnable() {
      @Override
      public void run() {
        compression.compressVideo(context, Arguments.createMap(), path, compressedVideoPath, new PromiseImpl(new Callback() {
          @Override
          public void invoke(Object... args) {
            String videoPath = (String) args[0];
